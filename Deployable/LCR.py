...
class MainMenu(tk.Tk):
    def __init__(self):
        ...
        #rules notes, negative values (disallow)
    ...
    def load_file(self):
        ...
        if file_path:
            ...
            self.selected_file_label.config(text="Selected File: " + os.path.basename(file_path), font=label_font)
...
class StartPage(tk.Toplevel):
    def __init__(self, parent, date_time_obj, data_handler):
        ...
        self.geometry("1650x510")  # Adjust the size as needed
        ...
        button4 = tk.Button(button_frame, text="Instant Market Shock", width=20, height=1, command=self.open_market_shock)
        button5 = tk.Button(button_frame, text="New Shock/Haircut", width=20, height=1,
        ...
        data = {
            '<HLAs>': ['Total HLA', 'Cash', 'Demand Deposit', 'Redeemable GIC', 'ST Money Market', '', '', '', '', '', ''],
            '<Exposure HLA ($m)>': [round(o[24]/1e6), round(o[25] / 1e6), round(o[26] / 1e6), round(o[27] / 1e6), round(o[28] / 1e6),
                               '', '', '', '', '', ''],
            '<LA Before Shock>': ['Total LA', 'Repo', 'Term Deposit 1M', 'Non-Redeemable GIC 1M',
                      'Term Deposit 3M',
                      "Non-Redeemable GIC 3M", "Passive Equities", "Passive FI Nominal",
                      "Passive FI Long Duration", "Passive Corporate Credit", ''],
            '<Exposure LA ($m)>': [round(o[29]/1e6), round(o[30] / 1e6), round(o[31] / 1e6), round(o[32] / 1e6), round(o[33] / 1e6),
                              round(o[34] / 1e6), round(o[35] / 1e6), round(o[36] / 1e6), round(o[37] / 1e6),
                              round(o[38] / 1e6), ''],
            '<IA Before Shock/Haircut>': ['Total IA', 'Active ARS', 'Active Equities', 'Active FI Nominal',
                      'Active FI Long Duration', 'Active Corporate Credit', '', '', '', '', ''],
            '<Exposure IA ($m)>': [round(o[39]/1e6), round(o[40] / 1e6), round(o[41] / 1e6), round(o[42] / 1e6),
                              round(o[43] / 1e6), round(o[44] / 1e6), '', '', '', '', ''],
            '<Derivative Notionals>': ['Total Derivative Notional','Currencies', 'Equities', 'FI Nominal','FI Long Duration',
                                       'Corporate Credit', '', '', '', '', ''],
            '<Exposure DN ($m)>': [round(o[50]/1e6), round(o[51] / 1e6), round(o[52] / 1e6), round(o[53] / 1e6),
                              round(o[54] / 1e6), round(o[55] / 1e6), '', '', '', '', '']
        }
        df = pd.DataFrame(data)
        df = df.applymap(
            lambda x: f"{locale.format_string('%.0f', x, grouping=True)}" if isinstance(x, int) or isinstance(x,
                                                                                                                float) else x)
        ...
        slr_lcr_row = ['<SLR>', '<SLR Values ($m)>', '<Inflows>', '<CI Values ($m)>', 
                       '<Outflows Before Buffer>', '<CO Values ($m)>',
                       '<LCR>', '<LCR Values>', '', '', '', '', '', '', '']
        ...
        row_5d = ['SLR 0-5D',  locale.format_string('%.0f', round(o[45] / 1e6), grouping=True),
                  'CI 0-5D',  locale.format_string('%.0f', round(o[61] / 1e6), grouping=True),
                  'CO 0-5D', ('-' if round(o[66] / 1e6) < 0 else '') + locale.format_string('%.0f', abs(round(o[66] / 1e6)), grouping=True),
                  'LCR 5D', locale.format_string('%.2f', round(o[0], 2)), '', '', '', '', '', '']
        self.treeview.insert('', 'end', values=row_5d)

        row_10d = ['SLR 6-10D', locale.format_string('%.0f', round(o[46] / 1e6), grouping=True) ,
                   'CI 6-10D',  locale.format_string('%.0f', round(o[62] / 1e6), grouping=True),
                   'CO 6-10D', ('-' if round(o[67] / 1e6) < 0 else '')  + locale.format_string('%.0f', abs(round(o[67] / 1e6)), grouping=True),
                   'LCR 10D', locale.format_string('%.2f', round(o[1], 2)), '', '', '', '', '', '']
        self.treeview.insert('', 'end', values=row_10d)

        row_30d = ['SLR 11-30D',  str(round(o[47] / 1e6)) , 'CI 11-30D',  str(
            round(o[63] / 1e6)),
        ...
        row_90d = ['SLR 31-90D', locale.format_string('%.0f', round(o[48] / 1e6), grouping=True),
           'CI 31-90D',  locale.format_string('%.0f', round(o[64] / 1e6), grouping=True) ,
           'CO 31-90D', ('-' if round(o[69] / 1e6) < 0 else '') + locale.format_string('%.0f', abs(round(o[69] / 1e6)), grouping=True) ,
           'LCR 90D', locale.format_string('%.2f', round(o[3], 2)), '', '', '', '', '', '']
        self.treeview.insert('', 'end', values=row_90d)

        row_365d = ['SLR 91-365D',  locale.format_string('%.0f', round(o[49] / 1e6), grouping=True),
                    'CI 91-365D',  locale.format_string('%.0f', round(o[65] / 1e6), grouping=True) ,
                    'CO 91-365D', ('-' if round(o[70] / 1e6) < 0 else '') + locale.format_string('%.0f', abs(round(o[70] / 1e6)), grouping=True) ,
                    'LCR 365D', locale.format_string('%.2f', round(o[4], 2)), '', '', '', '', '', '']
...
class Reallocation(tk.Toplevel):
    def __init__(self, parent, expected_date_str, data_handler):
        tk.Toplevel.__init__(self, parent)
        self.title("Reallocation")
        self.expected_date_str = expected_date_str
        self.data_handler = data_handler
        self.geometry("1620x740")
        # label = tk.Label(self, text="Reallocations", font=("Arial", 16))
        # label.grid(row=0, column=0, pady=10, padx=10, columnspan=12)

        # HLAs
        self.cash = tk.DoubleVar()
        self.demandDeposit = tk.DoubleVar()
        self.redeemableGIC = tk.DoubleVar()
        self.short_term_money_market = tk.DoubleVar()

        # LAs
        self.repo = tk.DoubleVar()
        self.termDeposit_1d_1m = tk.DoubleVar()
        self.non_redeemableGIC_1d_1m = tk.DoubleVar()
        self.termDeposit_1m_3m = tk.DoubleVar()
        self.non_redeemableGIC_1m_3m = tk.DoubleVar()
        self.passiveEQ = tk.DoubleVar()
        self.passiveFIN = tk.DoubleVar()
        self.passiveFILD = tk.DoubleVar()
        self.passiveIGC = tk.DoubleVar()

        # IAs
        self.activeARS = tk.DoubleVar()
        self.activeEQ = tk.DoubleVar()
        self.activeFIN = tk.DoubleVar()
        self.activeFILD = tk.DoubleVar()
        self.corporateCredit = tk.DoubleVar()

        # privates
        self.activePrivateEQ = tk.DoubleVar()
        self.activeREITs = tk.DoubleVar()
        self.infra = tk.DoubleVar()
        self.privateDebt = tk.DoubleVar()

        # derivative notionals
        self.DN_currencies = tk.DoubleVar()
        self.DN_equities = tk.DoubleVar()
        self.DN_fin = tk.DoubleVar()
        self.DN_fild = tk.DoubleVar()
        self.DN_igc = tk.DoubleVar()
        ...
        input_label3 = tk.Label(self, text="Redeemable GIC")
        ...
        input_label4 = tk.Label(self, text="ST MM")
        ...
        # LAs
        input_label5 = tk.Label(self, text="Repo")
        ...
        input_labela = tk.Label(self, text="Term Deposit 1M")
        ...
        input_label6 = tk.Label(self, text="Non-Redeemable GIC 1M")
        ...
        input_labelb = tk.Label(self, text="Term Deposit 3M")
        ...
        input_labelc = tk.Label(self, text="Non-Redeemable GIC 3M")
        ...
        input_label9 = tk.Label(self, text="Passive Fixed Income Long Duration")
        ...
        comment_text5 = "(Input $ are in millions)"

        comment_label5 = tk.Label(self, text=comment_text5, fg="red")
        comment_label5.grid(row=7, column=4, padx=10, pady=5, sticky=tk.W)

        comment_text3 = "Note: Empty inputs are disallowed"

        comment_label3 = tk.Label(self, text=comment_text3, fg="red")
        comment_label3.grid(row=8, column=4, padx=10, pady=5, sticky=tk.W)

        comment_text4 = "(If empty, LCRs won't be calculated)"

        comment_label4 = tk.Label(self, text=comment_text4, fg="red")
        comment_label4.grid(row=9, column=4, padx=10, pady=5, sticky=tk.W)
        ...
        input_label12 = tk.Label(self, text="Active Fixed Income Nominal")
        ...
        input_label13 = tk.Label(self, text="Active Fixed Income Long Duration")
        ...
        input_label14 = tk.Label(self, text="Active Corporate Credit")
        ...
        input_label16 = tk.Label(self, text="Active Real Estate")
        ...
        input_label17 = tk.Label(self, text="Infrastructure")
        ...
        input_label18 = tk.Label(self, text="Private Debt")
        ...
        input_label22 = tk.Label(self, text="Fixed Income Long Duration")
        ...
        input_label23 = tk.Label(self, text="Corporate Credit")
        ...
        reset_button = tk.Button(self, text="Reset", command=self.reset_entries, width=10)
        reset_button.grid(row=11, column=1, pady=10)

        self.textbox1 = tk.Text(self, width=70, height=22)
        self.textbox1.grid(row=12, column=0, columnspan=8, padx=10, pady=10, sticky=tk.NSEW)

        self.textbox2 = tk.Text(self, width=30, height=22)
        self.textbox2.grid(row=12, column=8, columnspan=10, padx=10, pady=10, sticky=tk.NSEW)
        ...
        visualization_button = tk.Button(self, text="Threshold",
                                          command=self.show_visualizations, width=10)
        visualization_button.grid(row=11, column=2, pady=10)

    def reset_entries(self):
        self.cash.set(0.0)
        self.demandDeposit.set(0.0)
        self.redeemableGIC.set(0.0)
        self.short_term_money_market.set(0.0)

        self.repo.set(0.0)
        self.termDeposit_1d_1m.set(0.0)
        self.non_redeemableGIC_1d_1m.set(0.0)
        self.termDeposit_1m_3m.set(0.0)
        self.non_redeemableGIC_1m_3m.set(0.0)
        self.passiveEQ.set(0.0)
        self.passiveFIN.set(0.0)
        self.passiveFILD.set(0.0)
        self.passiveIGC.set(0.0)

        self.activeARS.set(0.0)
        self.activeEQ.set(0.0)
        self.activeFIN.set(0.0)
        self.activeFILD.set(0.0)
        self.corporateCredit.set(0.0)

        self.activePrivateEQ.set(0.0)
        self.activeREITs.set(0.0)
        self.infra.set(0.0)
        self.privateDebt.set(0.0)

        self.DN_currencies.set(0.0)
        self.DN_equities.set(0.0)
        self.DN_fin.set(0.0)
        self.DN_fild.set(0.0)
        self.DN_igc.set(0.0)
    ...
    def show_visualizations(self):
        ...
        #negative balances (try exception pairs)
        #user's perspective
        #
        ...
        sizes_hla = [float(size.replace("", "").replace("", "").replace(",", "")) if size else 0.0 for size in
                     exposure_hla]
        ...
        sizes_la = [float(size.replace("", "").replace("", "").replace(",", "")) if size else 0.0 for size in
                    exposure_la]
    ...
    def submit_inputs(self):
        try:
            ...
            lcr5, lcr10, lcr30, lcr90, lcr365, lcr5_c, lcr10_c, lcr30_c, lcr90_c, lcr365_c, \
        lcr5_num, lcr5_denom, lcr10_num, lcr10_denom, lcr30_num, lcr30_denom, lcr90_num, \
        lcr90_denom, lcr365_num, lcr365_denom, lcr5_numc, lcr5_denomc, lcr10_numc, lcr10_denomc, \
        lcr30_numc, lcr30_denomc, lcr90_numc, lcr90_denomc, lcr365_numc, lcr365_denomc = self.reallocations()

            # Check if any of the calculated LCRs are negative
            if (lcr5_c < 0 or lcr10_c < 0 or lcr30_c < 0 or lcr90_c < 0 or lcr365_c < 0):
                raise ValueError("Negative Balance(s).")
            ...
            self.textbox2.insert(tk.END,
                                f"LCR 5: {locale.format_string('%.3f', lcr5)} | {locale.format_string('%.3f', lcr5_c)}\n")
            self.textbox2.insert(tk.END,
                                f"LCR 10: {locale.format_string('%.3f', lcr10)} | {locale.format_string('%.3f', lcr10_c)}\n")
            self.textbox2.insert(tk.END,
                                f"LCR 30: {locale.format_string('%.3f', lcr30)} | {locale.format_string('%.3f', lcr30_c)}\n")
            self.textbox2.insert(tk.END,
                                f"LCR 90: {locale.format_string('%.3f', lcr90)} | {locale.format_string('%.3f', lcr90_c)}\n")
            self.textbox2.insert(tk.END,
                                f"LCR 365: {locale.format_string('%.3f', lcr365)} | {locale.format_string('%.3f', lcr365_c)}\n\n")


            self.textbox2.insert(tk.END, "*LCR Numerators*\n")
            self.textbox2.insert(tk.END,
                            f"5D: {locale.format_string('%.0f', lcr5_num, grouping=True)} | {locale.format_string('%.0f', lcr5_numc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"10D: {locale.format_string('%.0f', lcr10_num, grouping=True)} | {locale.format_string('%.0f', lcr10_numc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"30D: {locale.format_string('%.0f', lcr30_num, grouping=True)} | {locale.format_string('%.0f', lcr30_numc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"90D: {locale.format_string('%.0f', lcr90_num, grouping=True)} | {locale.format_string('%.0f', lcr90_numc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"365D: {locale.format_string('%.0f', lcr365_num, grouping=True)} | {locale.format_string('%.0f', lcr365_numc, grouping=True)}\n\n")


            self.textbox2.insert(tk.END, "*LCR Denominators*\n")
            self.textbox2.insert(tk.END,
                            f"5D: {locale.format_string('%.0f', lcr5_denom, grouping=True)} | {locale.format_string('%.0f', lcr5_denomc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"10D: {locale.format_string('%.0f', lcr10_denom, grouping=True)} | {locale.format_string('%.0f', lcr10_denomc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"30D: {locale.format_string('%.0f', lcr30_denom, grouping=True)} | {locale.format_string('%.0f', lcr30_denomc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"90D: {locale.format_string('%.0f', lcr90_denom, grouping=True)} | {locale.format_string('%.0f', lcr90_denomc, grouping=True)}\n")
            self.textbox2.insert(tk.END,
                            f"365D: {locale.format_string('%.0f', lcr365_denom, grouping=True)} | {locale.format_string('%.0f', lcr365_denomc, grouping=True)}\n\n")
        ...
        # Display the error message
            self.textbox2.delete(1.0, tk.END)
            self.textbox2.insert(tk.END, f"Error: Negative Balance disallowed.")
    ...
    def reallocations(self):
        ...
        ################### get changes in $ #####################
        # HLA
        cash_c = self.cash.get()*1e6 + cash
        demandDeposit_c = self.demandDeposit.get()*1e6 + demandDeposit
        redeemableGIC_c = self.redeemableGIC.get()*1e6 + redeemableGIC
        short_term_money_market_c = self.short_term_money_market.get()*1e6 + short_term_money_market

        # LA
        repo_c = self.repo.get()*1e6 + repo
        termDeposit_1d_1m_c = self.termDeposit_1d_1m.get()*1e6 + termDeposit_1d_1m
        non_redeemableGIC_1d_1m_c = self.non_redeemableGIC_1d_1m.get()*1e6 + non_redeemableGIC_1d_1m
        termDeposit_1m_3m_c = self.termDeposit_1m_3m.get() + termDeposit_1m_3m
        non_redeemableGIC_1m_3m_c = self.non_redeemableGIC_1m_3m.get()*1e6 + non_redeemableGIC_1m_3m
        passiveEQ_c = self.passiveEQ.get()*1e6 + passiveEQ
        passiveFIN_c = self.passiveFIN.get()*1e6 + passiveFIN  # DN -FIN
        passiveFILD_c = self.passiveFILD.get()*1e6 + passiveFILD  # DN -FILD
        passiveIGC_c = self.passiveIGC.get()*1e6 + passiveIGC

        # IA
        activeARS_c = self.activeARS.get()*1e6 + activeARS
        activeEQ_c = self.activeEQ.get()*1e6 + activeEQ
        activeFIN_c = self.activeFIN.get()*1e6 + activeFIN  # DN -FIN
        activeFILD_c = self.activeFILD.get()*1e6 + activeFILD  # DN -FILD
        corporateCredit_c = self.corporateCredit.get()*1e6 + corporateCredit  # DN -IGC

        if (cash_c < 0 or demandDeposit_c < 0 or redeemableGIC_c < 0 or
            short_term_money_market_c < 0 or repo_c < 0 or termDeposit_1d_1m_c < 0 or
            non_redeemableGIC_1d_1m_c < 0 or termDeposit_1m_3m_c < 0 or
            non_redeemableGIC_1m_3m_c < 0 or passiveEQ_c < 0 or passiveFIN_c < 0 or
            passiveFILD_c < 0 or passiveIGC_c < 0 or activeARS_c < 0 or activeEQ_c < 0 or
            activeFIN_c < 0 or activeFILD_c < 0 or corporateCredit_c < 0):
            raise ValueError("One or more calculated variables are negative.")
        ...
        # original LCRs
        lcr5_num = df_filtered[5]
        lcr5_denom = df_filtered[6]
        lcr10_num = df_filtered[7]
        lcr10_denom = df_filtered[8]
        lcr30_num = df_filtered[9]
        lcr30_denom = df_filtered[10]
        lcr90_num = df_filtered[11]
        lcr90_denom = df_filtered[12]
        lcr365_num = df_filtered[13]
        lcr365_denom = df_filtered[14]

        lcr5_numc = (HLA + ci_5)
        lcr5_denomc = (slr_5d_c - co_5)
        lcr10_numc = (HLA + ci_5 + ci_10)
        lcr10_denomc = (slr_10d_c - (co_5 + co_10))
        lcr30_numc = (HLA + la_30_c + ci_5 + ci_10 + ci_30)
        lcr30_denomc = (slr_30d_c - (co_5 + co_10) - (co_30) * 1.2)
        lcr90_numc = (HLA + la_90_c + ci_5 + ci_10 + ci_30 + ci_90)
        lcr90_denomc = (slr_90d_c -
                                                                    (co_5 + co_10) - (co_30 + co_90) * 1.2)
        lcr365_numc = (HLA + la_365_c + ia_365_c + ci_5 + ci_10 + ci_30 + ci_90 \
                    + ci_365)
        
        lcr365_denomc = (slr_365d_c - (co_5 + co_10) - (co_30 + co_90 + co_365) * 1.2)
        ...
        lcr5_num, lcr5_denom, lcr10_num, lcr10_denom, lcr30_num, lcr30_denom, lcr90_num, \
        lcr90_denom, lcr365_num, lcr365_denom, lcr5_numc, lcr5_denomc, lcr10_numc, lcr10_denomc, \
        lcr30_numc, lcr30_denomc, lcr90_numc, lcr90_denomc, lcr365_numc, lcr365_denomc
...
class Shock_Haircut_Change(tk.Toplevel):
    def __init__(self, parent, expected_date_str, data_handler):
        tk.Toplevel.__init__(self, parent)
        self.title("New Shock/Haircut")
        self.expected_date_str = expected_date_str
        self.data_handler = data_handler
        self.geometry("1525x740")

        #shock 0-5 days
        self.equities_0_5 = tk.DoubleVar()
        self.equities_0_5.set(15)
        self.fin_0_5 = tk.DoubleVar()
        self.fin_0_5.set(5)
        self.fild_0_5 = tk.DoubleVar()
        self.fild_0_5.set(10)
        self.ars_0_5 = tk.DoubleVar()
        self.ars_0_5.set(15)
        self.igc_0_5 = tk.DoubleVar()
        self.igc_0_5.set(15)
        self.currency_0_5 = tk.DoubleVar()
        self.currency_0_5.set(5)

        #shock 6-10 days
        self.equities_6_10 = tk.DoubleVar()
        self.equities_6_10.set(15)
        self.fin_6_10 = tk.DoubleVar()
        self.fin_6_10.set(5)
        self.fild_6_10 = tk.DoubleVar()
        self.fild_6_10.set(10)
        self.ars_6_10 = tk.DoubleVar()
        self.ars_6_10.set(15)
        self.igc_6_10 = tk.DoubleVar()
        self.igc_6_10.set(15)
        self.currency_6_10 = tk.DoubleVar()
        self.currency_6_10.set(5)

        #shock 11-30 days
        self.equities_11_30 = tk.DoubleVar()
        self.equities_11_30.set(15)
        self.fin_11_30 = tk.DoubleVar()
        self.fin_11_30.set(5)
        self.fild_11_30 = tk.DoubleVar()
        self.fild_11_30.set(10)
        self.ars_11_30 = tk.DoubleVar()
        self.ars_11_30.set(15)
        self.igc_11_30 = tk.DoubleVar()
        self.igc_11_30.set(15)
        self.currency_11_30 = tk.DoubleVar()
        self.currency_11_30.set(5)

        #shock 31-90 days
        self.equities_31_90 = tk.DoubleVar()
        self.equities_31_90.set(25)
        self.fin_31_90 = tk.DoubleVar()
        self.fin_31_90.set(5)
        self.fild_31_90 = tk.DoubleVar()
        self.fild_31_90.set(15)
        self.ars_31_90 = tk.DoubleVar()
        self.ars_31_90.set(25)
        self.igc_31_90 = tk.DoubleVar()
        self.igc_31_90.set(25)
        self.currency_31_90 = tk.DoubleVar()
        self.currency_31_90.set(10)

        #shock 91-365 days
        self.equities_1y = tk.DoubleVar()
        self.equities_1y.set(40)
        self.fin_1y = tk.DoubleVar()
        self.fin_1y.set(10)
        self.fild_1y = tk.DoubleVar()
        self.fild_1y.set(25)
        self.ars_1y = tk.DoubleVar()
        self.ars_1y.set(40)
        self.igc_1y = tk.DoubleVar()
        self.igc_1y.set(40)
        self.currency_1y = tk.DoubleVar()
        self.currency_1y.set(20)

        #haircuts
        self.equities_hc = tk.DoubleVar()
        self.equities_hc.set(4)
        self.fin_hc = tk.DoubleVar()
        self.fin_hc.set(3)
        self.fild_hc = tk.DoubleVar()
        self.fild_hc.set(3)
        self.ars_hc = tk.DoubleVar()
        self.ars_hc.set(4)
        self.igc_hc = tk.DoubleVar()
        self.igc_hc.set(4)
        self.currency_hc = tk.DoubleVar()


        #input labels
        adj_label0 = tk.Label(self, text="Asset Classes", font=("Helvetica", 12, "bold"))
        ...
        comment_text = "Positive input to shock/haircut means drawdown %"

        comment_label = tk.Label(self, text=comment_text, fg="red")
        comment_label.grid(row=9, column=0, padx=10, pady=5, sticky=tk.W)

        comment_text2 = "Shock/Haircut inputs are replacement of frameworks'"

        comment_label2 = tk.Label(self, text=comment_text2, fg="red")
        comment_label2.grid(row=8, column=0, padx=10, pady=5, sticky=tk.W)

        comment_text3 = "Note: Empty inputs are disallowed"

        comment_label3 = tk.Label(self, text=comment_text3, fg="red")
        comment_label3.grid(row=10, column=0, padx=10, pady=5, sticky=tk.W)
        ...
        inp_lb2 = tk.Label(self, text="FI Nominal")
        ...
        inp_lb3 = tk.Label(self, text="FI Long Duration")
        ...
        inp_lb5 = tk.Label(self, text="Corporate Credit")
        ...
        self.textbox2 = tk.Text(self, width=37, height=22)
        self.textbox2.grid(row=12, column=8, columnspan=13, padx=10, pady=10, sticky=tk.NSEW)

        scrollbar = tk.Scrollbar(self, command=self.textbox1.yview)
        scrollbar.grid(row=12, column=23, rowspan=3, sticky=tk.NS)
        self.textbox1.config(yscrollcommand=scrollbar.set)
        ...
        df = pd.DataFrame(data)
        df = df.applymap(
            lambda x: f"{locale.format_string('%.0f', x, grouping=True)}" 
    ...
    def submit_inputs(self):
        ...
        lcr5_num, lcr5_denom, lcr10_num, lcr10_denom, lcr30_num, lcr30_denom, lcr90_num, \
        lcr90_denom, lcr365_num, lcr365_denom, lcr5_numc, lcr5_denomc, lcr10_numc, lcr10_denomc, \
        lcr30_numc, lcr30_denomc, lcr90_numc, lcr90_denomc, lcr365_numc, lcr365_denomc = self.shock_hc()
    ...
    def shock_hc(self):
        ...
        lcr5_numc = (HLA + ci_5)
        lcr5_denomc = (slr_updated_5d - co_5)
        lcr10_numc = (HLA + ci_5 + ci_10)
        lcr10_denomc = (slr_updated_10d - (co_5 + co_10))
        lcr30_numc = (HLA + la_updated_30 + ci_5 + ci_10 + ci_30)
        lcr30_denomc = (slr_updated_30d - (co_5 + co_10) - (co_30) * 1.2)
        lcr90_numc = (HLA + la_updated_90 + ci_5 + ci_10 + ci_30 + ci_90)
        lcr90_denomc = (slr_updated_90d -
                                                                    (co_5 + co_10) - (co_30 + co_90) * 1.2)
        lcr365_numc = (HLA + la_updated_1y + ia_updated_1y + ci_5 + ci_10 + ci_30 + ci_90 \
                    + ci_365)
        
        lcr365_denomc = (slr_updated_1y - (co_5 + co_10) - (co_30 + co_90 + co_365) * 1.2)
...
class MarketShock(tk.Toplevel):
    def __init__(self, parent, expected_date_str, data_handler):
        tk.Toplevel.__init__(self, parent)
        self.title("Instant Market Shock")
        self.expected_date_str = expected_date_str
        self.data_handler = data_handler
        ...
        input_label2 = tk.Label(self, text="Enter market shock % for fixed income nominal (+/-)")
        ...
        input_label3 = tk.Label(self, text="Enter market shock % for fixed income long duration (+/-)")
        ...
        input_label5 = tk.Label(self, text="Enter market shock % for corporate credit (+/-)")
        ...
        # Comment 1
        comment_label1 = tk.Label(self, text="Assume derivative notional $ is constant, but MTM from shock triggers inflow/outflow", foreground="red")
        comment_label1.pack(pady=5, padx=10, anchor=tk.W)

        # Comment 2
        comment_label2 = tk.Label(self, text="Shocks are incremental to policy's", foreground="red")
        comment_label2.pack(pady=5, padx=10, anchor=tk.W)
    ...
    def submit_inputs(self):
        ...
        self.textbox.insert(tk.END,
                            f"Corporate Credit %: {locale.format_string('%.0f', self.investmentGradeCredit.get(), grouping=True)}\n")
        self.textbox.insert(tk.END,
        ...
        self.textbox.insert(tk.END,
                            f"LCR 5: {locale.format_string('%.3f', lcr5)} | {locale.format_string('%.3f', new_lcr_5)}\n")
        self.textbox.insert(tk.END,
                            f"LCR 10: {locale.format_string('%.3f', lcr10)} | {locale.format_string('%.3f', new_lcr_10)}\n")
        self.textbox.insert(tk.END,
                            f"LCR 30: {locale.format_string('%.3f', lcr30)} | {locale.format_string('%.3f', new_lcr_30)}\n")
        self.textbox.insert(tk.END,
                            f"LCR 90: {locale.format_string('%.3f', lcr90)} | {locale.format_string('%.3f', new_lcr_90)}\n")
        self.textbox.insert(tk.END,
                            f"LCR 365: {locale.format_string('%.3f', lcr365)} | {locale.format_string('%.3f', new_lcr_365)}\n\n")

        # cash assets after shock
        self.textbox.insert(tk.END, "**Cash Assets (before shock | after shock)**\n")
        self.textbox.insert(tk.END,
                            f"Equities: {locale.format_string('%.0f', equities, grouping=True)} | {locale.format_string('%.0f', eq_initial, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"FI Nominal: {locale.format_string('%.0f', FIN, grouping=True)} | {locale.format_string('%.0f', fin_initial, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"FI Long Duration: {locale.format_string('%.0f', FILD, grouping=True)} | {locale.format_string('%.0f', fild_initial, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"Corporate Credit: {locale.format_string('%.0f', IGC, grouping=True)} | {locale.format_string('%.0f', igc_initial, grouping=True)}\n")

        self.textbox.insert(tk.END,
                            f"ARS: {locale.format_string('%.0f', ARS, grouping=True)} | {locale.format_string('%.0f', ars_initial, grouping=True)}\n\n")

        # derivative notionals
        self.textbox.insert(tk.END, "**Derivative Notional**\n")
        self.textbox.insert(tk.END,
                            f"Equity: {locale.format_string('%.0f', EQ_hedge, grouping=True)}| Incremental MTM before Buffer: {locale.format_string('%.0f', MTM_eq, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"Fixed Income Nominal: {locale.format_string('%.0f', FIN_hedge, grouping=True)} | Incremental MTM before Buffer: {locale.format_string('%.0f', MTM_fin, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"Fixed Income Long Duration: {locale.format_string('%.0f', FILD_hedge, grouping=True)} | Incremental MTM before Buffer: {locale.format_string('%.0f', MTM_fild, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"Corporate Credit: {locale.format_string('%0.f', IGC_hedge, grouping=True)} | Incremental MTM before Buffer: {locale.format_string('%.0f', MTM_igc, grouping=True)}\n")
        self.textbox.insert(tk.END,
                            f"Currency: {locale.format_string('%.0f', currency_hedge, grouping=True)} | Incremental MTM before Buffer: {locale.format_string('%.0f', MTM_fx, grouping=True)}\n")
...
class CashOutflowBuffer(tk.Toplevel):
    def __init__(self, parent, expected_date_str, data_handler):
        tk.Toplevel.__init__(self, parent)
        self.title("Cash Outflow Buffer Change")
        self.expected_date_str = expected_date_str
        self.data_handler = data_handler
        self.geometry("1265x590")
        ...
        self.three_zero.set(20)
        self.nine_zero = tk.IntVar()
        self.nine_zero.set(20)
        self.year = tk.IntVar()
        self.year.set(20)

        #input labels
        adj_label0 = tk.Label(self, text="Timeframes", font=("Helvetica", 12, "bold"))
        ...
        comment_text = "Buffers (%) are not incremental to frameworks'"
        adj_lb1 = tk.Label(self, text="", font=("Arial", 12, "bold"))
        adj_lb1.grid(row=1, column=2, padx=10, pady=5, sticky=tk.W, columnspan=4)

        comment_label = tk.Label(self, text=comment_text, fg="red")
        comment_label.grid(row=1, column=5, padx=10, pady=5, sticky=tk.W)


        comment_text2 = "Note: Empty inputs are disallowed"
        adj_lb12 = tk.Label(self, text="", font=("Arial", 12, "bold"))
        adj_lb12.grid(row=2, column=3, padx=10, pady=5, sticky=tk.W, columnspan=4)

        comment_label2 = tk.Label(self, text=comment_text2, fg="red")
        comment_label2.grid(row=2, column=5, padx=10, pady=5, sticky=tk.W)
        ...
        #additional formatting
        submit_button = tk.Button(self, text="Enter", command=self.submit_inputs, width=10)
        submit_button.grid(row=7, column=0, pady=10)
        self.bind("<Return>", lambda event: submit_button.invoke())
        ...
        self.textbox1 = tk.Text(self, width=80, height=20)
        self.textbox1.grid(row=11, column=0, columnspan=10, padx=10, pady=10, sticky=tk.NSEW)

        self.textbox2 = tk.Text(self, width=37, height=10)
        self.textbox2.grid(row=11, column=10, columnspan=13, padx=10, pady=10, sticky=tk.NSEW)

        scrollbar = tk.Scrollbar(self, command=self.textbox1.yview)
        scrollbar.grid(row=11, column=24, rowspan=3, sticky=tk.NS)
        self.textbox1.config(yscrollcommand=scrollbar.set)
        ...
        locale.setlocale(locale.LC_ALL, '')
        data = {
            '<Inflows>': ['0-5 Days', '6-10 Days', '11-30 Days', '31-90 Days', '91-365 Days'],
            '<CI Values ($m)>': [round(o[61])/ 1e6, round(o[62])/ 1e6, round(o[63])/ 1e6, round(o[64])/ 1e6, round(o[65])/ 1e6],
            '<Outflows Before Buffer>': ['0-5 Days', '6-10 Days', '11-30 Days', '31-90 Days', '91-365 Days'],
            '<CO Values ($m)>': [round(o[66]) / 1e6, round(o[67]) / 1e6, round(o[68]) / 1e6, round(o[69]) / 1e6, round(o[70]) / 1e6],
            '<Net Flow Before Buffer': ['0-5 Days', '6-10 Days', '11-30 Days', '31-90 Days', '91-365 Days'],
            '<Net Flow Values ($m)': [round(o[56]) / 1e6, round(o[57]) / 1e6, round(o[58]) / 1e6, round(o[59]) / 1e6, round(o[60]) / 1e6]
        }

        df = pd.DataFrame(data)
        df = df.applymap(lambda x: 
            f"{locale.format_string('%.0f', x, grouping=True)}" if isinstance(x, int) or isinstance(x, float) else x)
        
        # Handle negative numbers
        # df = df.applymap(lambda x: 
        #     f"-{x[2:]}" if isinstance(x, str) and x.startswith('-') else x)
        ...
        self.treeview.grid(row=11, column=0, columnspan=10, padx=10, pady=10, sticky=tk.NSEW)
        self.formatted_df = self.format_dataframe()
